...
import (
	...
	"github.com/shaft/go-shaft/common/mclock"
...
// Service implements an SHAFT netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	eth    *eth.SHAFT      // Full SHAFT service if monitoring a full node
	les    *les.LightSHAFT // Light SHAFT service if monitoring a light node
...
func New(url string, ethServ *eth.SHAFT, lesServ *les.LightSHAFT) (*Service, error) {
