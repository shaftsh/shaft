...
type Config struct {
	Oracle common.Address // SHAFT address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the SHAFT service dependency to access the blockchain
	var apiBackend ethapi.Backend
	var shaft *eth.SHAFT
	if err := ctx.Service(&shaft); err == nil {
		apiBackend = shaft.ApiBackend
	} else {
		var shaft *les.LightSHAFT
		if err := ctx.Service(&shaft); err == nil {
			apiBackend = shaft.ApiBackend
		} else {
			return nil, err
...
func (r *ReleaseService) checkVersion() {
	...
	if version.Major > r.config.Major ||
		...
		howtofix := fmt.Sprintf("Please check https://github.com/shaft/go-shaft/releases for new releases")
