...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the SHAFT protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for SHAFT and the faucet")
)
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // SHAFT address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an SHAFT light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // SHAFT protocol stack
	client *ethclient.Client   // Client connection to the SHAFT chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the SHAFT light client protocol
...
// close terminates the SHAFT connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for SHAFT addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No SHAFT address found to fund"})
			continue
		}
