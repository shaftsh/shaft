...
// nodeDockerfile is the Dockerfile required to run an SHAFT node.
var nodeDockerfile = `
FROM shaft/client-go:alpine-develop
...
RUN \
  echo '/geth init /genesis.json' > geth.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.shaft/keystore/ && cp /signer.json /root/.shaft/keystore/' >> geth.sh && \{{end}}
...
// an SHAFT node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
...
// deployNode deploys a new SHAFT node container to a remote machine via SSH,
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.shaft"],
