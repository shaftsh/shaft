...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/shaft                                                            && \
	(cd $GOPATH/src/github.com/shaft && git clone --depth=1 https://github.com/shaft/go-shaft) && \
  go build -v github.com/shaft/go-shaft/cmd/faucet                                              && \
