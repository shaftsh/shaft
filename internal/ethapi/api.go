...
import (
	...
	"github.com/shaft/go-shaft/accounts/keystore"
	"github.com/shaft/go-shaft/common"
	"github.com/shaft/go-shaft/common/hexutil"
	"github.com/shaft/go-shaft/common/math"
	...
	"github.com/shaft/go-shaft/crypto"
...
// PublicSHAFTAPI provides an API to access SHAFT related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicSHAFTAPI struct {
	b Backend
}

// NewPublicSHAFTAPI creates a new Etheruem protocol API.
func NewPublicSHAFTAPI(b Backend) *PublicSHAFTAPI {
	return &PublicSHAFTAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicSHAFTAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current SHAFT protocol version this node supports
func (s *PublicSHAFTAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicSHAFTAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19SHAFT Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19SHAFT Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an SHAFT ECDSA signature for:
// keccack256("\x19SHAFT Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/shaft/go-shaft/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19SHAFT Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/shaft/go-shaft/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid SHAFT signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the SHAFT blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
...
//
// https://github.com/shaft/wiki/wiki/JSON-RPC#eth_sign
...
// Version returns the current shaft protocol version.
