...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.shaft", new NodeConfig());
			node.start();
			...
			SHAFTClient ec = node.getSHAFTClient();
	...
	// signatures too. Regression test for go-shaft issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/shaft/gethtest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.shaft", "github.com/shaft/go-shaft/mobile")
	...
	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "shaft", "gethtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
