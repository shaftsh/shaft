...
// NewProtocolManager returns a new shaft sub protocol manager. The SHAFT sub protocol manages peers capable
// with the shaft network.
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing SHAFT peer", "peer", id)

	// Unregister the peer from the downloader and SHAFT peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping SHAFT protocol")
	...
	log.Info("SHAFT protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("SHAFT peer connected", "name", p.Name())

	// Execute the SHAFT handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("SHAFT handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("SHAFT peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("SHAFT message handling failed", "err", err)
			return err
		}
	}
}
...
// EthNodeInfo represents a short summary of the SHAFT sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    uint64      `json:"network"`    // SHAFT network ID (1=Frontier, 2=Morden, Ropsten=3)
